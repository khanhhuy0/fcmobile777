...
function newSeason(init=false){
 ...
 // 👉 ép tính lần đầu
 recalcSeason(seasons[seasons.length-1]);
...
function calcGlobalTitles(){
 ...
 🏆 Vua vô địch: ${max("cup")[0]} (${max("cup")[1].cup})<br>
...
function endSeason(){
 ...
 playChampionMusic();
 startFireworks(name); // ✅ TRUYỀN TÊN CLB
}
